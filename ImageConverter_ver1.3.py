...
# def sys_select(op):
#     global sys_ivs800
#     sys_type = variable.get()
#     if sys_type == 'IVS-800': sys_ivs800 = True
#     elif sys_type == 'IVS-2000-HR': sys_ivs800 = False
#     else: ValueError('Wrong system type')
#     tk_par.destroy()
# choices = ['IVS-800', 'IVS-2000-HR']
# tk_par = Tk();variable = StringVar(tk_par);variable.set(choices[0])
# popupMenu = OptionMenu(tk_par, variable, *choices, command=sys_select);
# Label(tk_par, text="Choose the used system").grid(row=0, column=0); popupMenu.grid(row=1, column=0)
# tk_par.mainloop()
...
if sys_ivs800:
    octRangedB = [-25, 20]
# # # - - - - use TkInterface to catch dir path of data file - - - - # # #
tk = Tk(); tk.withdraw(); DataFold = filedialog.askopenfilename(filetypes=[("", "*")])
...
if save_tif:
    ...
    if gpu_proc: tifffile.imwrite(root + '\\' + DataId[:-4] + '_IntImg.tif', np.rollaxis(res_octImgVol[:, :, :], 0,1).get().astype(dtype='float32'))#, compression='zlib', compressionargs={'level': 8})
    else: tifffile.imwrite(root + '\\' + DataId[:-4] + '_IntImg.tif', np.rollaxis(res_octImgVol[:, :, :], 0,1).astype(dtype='float32'))
if save_video and dataType == 'timelapse':
