...
DataId = os.path.basename(stackFilePath);   root = os.path.dirname(DataId);  tk.destroy()
...
hueRange = [0, 0.10]  # variance: 0~0.10 / std: 0~
...
for depthIndex in range(dim_z):   # dim_z
    ...
    varProj_hue = np.multiply(np.clip((varProj_var - hueRange[0]) / (hueRange[1] - hueRange[0]), 0, 1), 0.6)  # limit color display range from red to blue
